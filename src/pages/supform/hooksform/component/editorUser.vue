<template>
    <!-- addUser -->
    <el-dialog v-model="props.statusShow" title="修改用户" width="30%">
        <el-form ref="ruleFormRef" :model="ruleForm" status-icon :rules="rules" label-width="120px" class="demo-ruleForm">
            ...
            <el-form-item label="居住地址：" prop="address">
                <el-input v-model="ruleForm.address" placeholder="请输入居住地址" maxlength="40"/>
            </el-form-item>
        </el-form>
        <template #footer>
...
<script setup lang="ts">
    ...
    watch(
        ...
        (newVal, oldVal) => {
            ...
            ruleForm.idCard = newVal.idCard
            ruleForm.email = newVal.email
            ruleForm.address = newVal.address
            ruleForm.user_id = newVal.user_id
        }, {
            deep: true
        }
    )
    ...
    const submitForm = (formEl: FormInstance | undefined) => {
        ...
        formEl.validate(async (valid) => {
            if (valid) {
                const res: any = await userHooksTableApi.updatehookstable(ruleForm)
                console.log(res)
                quxiao(formEl)
                if (res.status === 0) {
                    ElMessage({
